  ...
  <blockquote>
    <p>Administração de todos os Usuarios de Administração cadastrados!</p>
    <footer>Someone famous in <cite title="Source Title">Source Title</cite></footer>
    <a href="/tripshop/adicionar/cadastrar_admin">Novo Usuario <span class="glyphicon glyphicon-plus"></span></a>
  </blockquote>
  <div class="table-responsive">
    <table class="table table-striped table-fluid table-responsive">
                <tr>
                    <th>ID</th>
                    <th>Usuario</th>
                    <th>E-mail</th>
                    <th>Tipo Usuario</th>
                    <th class="table-iconcell">Ativo</th>
                ...
                <? foreach ($seleciona -> result() as $seleciona): ?>
                    <tr>
                        <td><?= $seleciona->id ?></td>
                        <td><?= $seleciona->usuario ?></td>
                        <td><?= $seleciona->email ?></td>
                        <td><? if ($seleciona->tipo_usuario == '1')
                                  echo "Usuario";
                               else if($seleciona->tipo_usuario == '2')
                                  echo "Vendedor";
                               else
                                  echo "Desenvolvedor";?></td>
                        <td><? if ($seleciona->ativo == '1') {
                          echo "<span class='glyphicon glyphicon-ok-circle green row-fluid'></span>";
                          }
                          else
                            echo "<span class='glyphicon glyphicon-remove-circle red row-fluid'></span>";?></td>
