      <script>
    	...
    	function carregaDadosJSon(id){
    		$.post(base_url+'admin/festas/dados', {
    			'id': id,
          ...
          $('#festa_atracao').val(data.festa_atracao);
      ...
      function deletar(id){
            ...
            $.post(base_url+'admin/festas/deletar', {
                id: id,
                '<?php echo $this->security->get_csrf_token_name(); ?>':'<?php echo $this->security->get_csrf_hash(); ?>'
            }, function (data){
                $('#id').val(data.id);//aqui eu seto a o input hidden com o id do cliente, para que a edição funcione. Em cada tela aberta, eu seto o id do cliente.
  ...
  <blockquote>
    <p>Administração de todos as Festas cadastrados!</p>
    <footer>Someone famous in <cite title="Source Title">Source Title</cite></footer>
    <a href="/tripshop/adicionar/cadastrar_festa">Novo festa <span class="glyphicon glyphicon-plus"></span></a>
  </blockquote>
    <table class="table table-striped table-fluid ">
                ...
                <? foreach ($seleciona -> result() as $seleciona): ?>
                    <tr>
                        <td><?= $seleciona->id ?></td>
                        <td><?= $seleciona->festa_atracao ?></td>
                        <td><?= $seleciona->casa ?></td>
                        <td><?= date("d-m-Y", strtotime($seleciona->data)) ?></td>
                        <td><?= $seleciona->hora ?></td>
	  ...
	  <div class="modal-dialog">
	    <div class="modal-content">
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title">Editar festa</h4>
	      </div>
          ...
          <?php
            ...
            echo form_open('admin/festas/salvar', $attributes);
